...
class ChatMessage(models.Model):
    ...
    timestamp = models.DateTimeField(
        verbose_name="Время отправки сообщения", default=timezone.now, db_index=True
    )

    is_read = models.BooleanField(
        verbose_name="Прочитано", default=False, db_index=True
    )
    ...
    def get_unread_count(cls, order, user):
        return (
            cls.objects.filter(order=order, is_read=False).exclude(sender=user).count()
        )
