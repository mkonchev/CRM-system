...
class WorkSerializerTest(TestCase):
    ...
    def test_work_serialization(self):
        self.assertEqual(self.serializer.data["name"], self.work_data["name"])
        self.assertEqual(
            self.serializer.data["description"], self.work_data["description"]
        )
        self.assertEqual(self.serializer.data["car"], self.work_data["car"].pk)
        self.assertEqual(self.serializer.data["price"], self.work_data["price"])

    def test_work_serializer_data(self):
        data = self.serializer.data
        self.assertEqual(data["name"], "Замена масла")
        self.assertEqual(data["description"], "Полная замена моторного масла и фильтра")
    ...
    def test_update_serializer(self):
        ...
        serializer = WorkSerializer(instance=self.work, data=update_data, partial=True)
        self.assertTrue(serializer.is_valid())
        work = serializer.save()
