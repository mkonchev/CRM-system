...
class WorkstatusSerializerlTest(TestCase):
    ...
    def test_workstatus_serialization(self):
        self.assertEqual(self.serializer.data["work"], self.workstatus_data["work"].pk)
        self.assertEqual(
            self.serializer.data["order"], self.workstatus_data["order"].pk
        )
        (
            self.assertEqual(
                self.serializer.data["status"], self.workstatus_data["status"]
            ),
        )
        (
            self.assertEqual(
                self.serializer.data["amount"], self.workstatus_data["amount"]
            ),
        )
        self.assertEqual(
            self.serializer.data["fix_price"], self.workstatus_data["fix_price"]
        )
    ...
    def test_update_serializer(self):
        ...
        serializer = WorkstatusSerializer(
            instance=self.workstatus, data=update_data, partial=True
        )
        self.assertTrue(serializer.is_valid())
        workstatus = serializer.save()
